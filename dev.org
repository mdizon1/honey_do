...
** TODO Separate syncing from client
   ...
   is the way to do it.
*** TODO Pull out a single api call
**** TODO Create sync buffer
**** TODO Where api call is made, instead push a command (action) into the sync buffer
**** TODO Create sync buffer class which exposes an api to push commands into the buffer and flushes the sync
