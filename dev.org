...
*** TODO There's also the problem of getTodosFromStore being called 4 times on startup
    ...
    Seems like it fires on todo click either way.  Ohhhh! got it! Since it
    triggers on every action, onTodoClick fires one action completeTodoRequest
    then the async action for completeTodoSuccess.
