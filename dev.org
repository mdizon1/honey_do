...
*** TODO Try having each todo item grab itself from store
    ...
    the relevant todos
**** TODO consolidate todo data state
...
***** TODO Edit todo should grab the todo from id only
***** TODO Try backing out the id thing.
      Where TodoList passes ids down, instead just pass the whole todo, but as immutables
...
*** TODO Big lag between check todo and todo getting checked
    First let's see if there's a big delay between the actual touch and the action being triggered
